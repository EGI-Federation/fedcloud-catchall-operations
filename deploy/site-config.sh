...
fedcloud openstack token issue --oidc-access-token "$OIDC_TOKEN" \
...
DEPLOY_OS_TOKEN="$(fedcloud openstack token issue \
...
fedcloud secret put --locker-token "$FEDCLOUD_LOCKER_TOKEN" \
